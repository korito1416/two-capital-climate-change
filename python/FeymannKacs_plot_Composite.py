...
# elif min(xij2arr)==0.150:

Filename = 'AversionDiff'
...
def model_simulation_generate(id, xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho,psi_0,psi_1,varrho):
    ...
    # for i in range(1,51):
        ...
        try:
            ...
            # print(i)
            simbatch = np.load(Data_Dir + File_Dir+'Sim_'+str(i)+f'_{IntPeriod}_{str(dt)}_{m0}_ID.npz')
    ...
    print(f"Second Year 0={Second_Term_0}, 30={Second_Term_30}, Ratio={Second_Term_30/Second_Term_0}")
        ...
        for id_psi1 in range(len(psi1arr)):
            for id_varrho in range(len(varrhoarr)):
                for id_rho in range(len(rhoarr)):
                    ...
                    res,  res2, res3, res4, time_process, res_ProbDamage, res_ProbTech, res_probdamage, res_probtech = model_simulation_generate(id_xiag, xiaarr[id_xiag],xikarr[id_xiag],xicarr[id_xiag],xijarr[id_xiag],xidarr[id_xiag],xigarr[id_xiag],rhoarr[id_rho],psi0arr[id_psi0],psi1arr[id_psi1],varrhoarr[id_varrho])
