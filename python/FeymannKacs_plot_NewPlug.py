...
# if min(xij2arr)==0.075 and min(xijarr)>1:
#     labellist = ['Pre More Aversion Post Neutrality']
#     Filename = 'Aversion Intensity_More'
#     print("define success")

# if min(xij2arr)==0.075 and min(xijarr)==0.075:
...
# if min(xij2arr)>1 and min(xijarr)==0.075:
#     labellist = ['Pre Neutrality Post More Aversion']
#     Filename = 'Aversion Intensity_More'
#     print("define success")
...
def model_simulation_generate(id, xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,xi_a2,xi_k2,xi_c2,xi_j2,xi_d2,xi_g2,rho,psi_0,psi_1,varrho):
    ...
    # print(time_process)
    plt.figure()
    plt.plot(time_process, np.mean(undiscount_processes, axis=0),label = r'$\delta m \cdot \frac{\partial U}{\partial x} + m \cdot \frac{\partial J}{\partial x} (V^j - V) + m \cdot J \frac{\partial V^j}{\partial x} $')
    plt.legend(loc='lower left')
    plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_UndiscountedProcess_dt.pdf")
    plt.close()


    plt.figure()
    plt.plot(time_process,  np.mean(discount_processes, axis=0),label = r'$\exp( \int_0^t(-\delta - J)du )(\delta m \cdot \frac{\partial U}{\partial x} + m \cdot \frac{\partial J}{\partial x} (V^j - V) + m \cdot J \frac{\partial V^j}{\partial x}) $')
    plt.legend(loc='lower left')
    plt.title(str(m0)+" "+labellist[id])
    if m0=="Technology":
        # plt.ylim(0,0.002)
    ...
    if m0=="Temperature":
        ...
        plt.xlim(0,50)
        print("ylim Applied Success")
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_DiscountedProcess_dt.pdf")
    plt.close()
    ...
    plt.figure()
    plt.plot(time_process, np.mean(delta*first_terms, axis=0),label = r'$\delta m \cdot \frac{\partial U}{\partial x}$')
    plt.plot(time_process, np.mean(second_terms, axis=0),label = r'$m \cdot \frac{\partial J}{\partial x} (V^j - V)$')
    ...
    plt.plot(time_process, np.mean(third_term_1s, axis=0)+np.mean(third_term_2s, axis=0)+np.mean(third_term_3s, axis=0)+np.mean(third_term_4s, axis=0),label = r'$m \cdot J \frac{\partial V^j}{\partial x} $')
    plt.plot(time_process, np.mean(fourth_term_entropys, axis=0),label = r'$\xi m \cdot \frac{\partial J}{\partial x} (1-g^* + g^* \log g^*)$')
    # plt.plot(,label = 'third term 3')
    plt.legend(loc='upper left')
    plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_Undiscount_Term1234_decomposition_dt.pdf")
    plt.close()


    plt.figure()
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes), axis=0),label = r'$\exp(\int_0^t (-\delta-J) dt)$')
    # plt.plot(,label = 'third term 3')
    plt.legend(loc='upper right')
    # plt.ylim(0,0.001)
    plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_Discount_Factor_dt.pdf")
    plt.close()
    ...
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(second_terms), axis=0),label = r'$m \cdot \frac{\partial J}{\partial x} (V^j - V)$')
    # plt.plot(np.mean(third_term_1s, axis=0),label = 'third term 1')
    # plt.plot(np.mean(third_term_2s, axis=0),label = 'third term 2')
    # plt.plot(np.mean(third_term_3s, axis=0),label = 'third term 3'
    ...
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(third_term_1s+third_term_2s+third_term_3s+third_term_4s), axis=0),label = r'$m \cdot J \frac{\partial V^j}{\partial x} $')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(fourth_term_entropys), axis=0),label = r'$\xi m \cdot \frac{\partial J}{\partial x} (1-g^* + g^* \log g^*)$')
    # plt.plot(,label = 'third term 3')
    plt.legend(loc='upper right')
    # plt.ylim(0,0.0015)
    plt.ylim(0,0.0014)
    # plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_Discount_Term1234_dt.pdf")
    plt.close()

    plt.figure()
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(delta*first_terms), axis=0),label = r'i')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(second_terms), axis=0),label = r'ii')
    # plt.plot(np.mean(third_term_1s, axis=0),label = 'third term 1')
    # plt.plot(np.mean(third_term_2s, axis=0),label = 'third term 2')
    # plt.plot(np.mean(third_term_3s, axis=0),label = 'third term 3'
    # plt.plot(np.mean(third_term_1s, axis=0)+np.mean(third_term_2s, axis=0)+np.mean(third_term_3s, axis=0)+np.mean(third_term_4s, axis=0),label = r'$m \cdot J \frac{\partial V^j}{\partial x} $')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(third_term_1s+third_term_2s+third_term_3s+third_term_4s), axis=0),label = r'iii')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(fourth_term_entropys), axis=0),label = r'iv')
    # plt.plot(,label = 'third term 3')
    plt.legend(loc='upper right')
    # plt.ylim(0,0.0015)
    plt.ylim(0,0.0014)
    ...
    # plt.plot(np.mean(third_term_3s, axis=0),label = 'third term 3'
    # plt.plot(np.mean(third_term_1s, axis=0)+np.mean(third_term_2s, axis=0)+np.mean(third_term_3s, axis=0)+np.mean(third_term_4s, axis=0),label = r'$m \cdot J \frac{\partial V^j}{\partial x} $')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(third_term_2s), axis=0),label = r'$m \cdot J \sum_1^{L-1} \frac{ \partial V^\ell}{\partial x} $')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(third_term_3s), axis=0),label = r'$m \cdot J \frac{\partial V^L}{\partial x} $')
    plt.plot(time_process, np.mean(np.exp(discount_factor_processes)*(fourth_term_entropys), axis=0),label = r'$\xi m \cdot \frac{\partial J}{\partial x} (1-g^* + g^* \log g^*)$')
    # plt.plot(,label = 'third term 3')
    plt.legend(loc='upper right')
    # plt.ylim(0,0.0015)
    plt.ylim(0,0.0014)
    # plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_Discount_Term123techdamage4_dt.pdf")
    plt.close()


    plt.figure()
    plt.plot(time_process, np.mean(np.exp(discount_factor_nodelta_DisSep_Damage_processes), axis=0),label = r'$Prob_{Damage}(t;x)$')
    plt.plot(time_process, np.mean(np.exp(discount_factor_nodelta_DisSep_Tech_processes), axis=0),label = r'$Prob_{Tech}(t;x)$')
    plt.legend(loc='upper right')
    # plt.ylim(0,0.0012)
    # plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_DiscountFactor_Probability_Sep.pdf")
    plt.close()



    plt.figure()
    plt.plot(time_process, np.mean(discount_factor_nodeltadt_DisSep_Damage_processes, axis=0),label = r'$prob_{Damage}(t;x)$')
    plt.plot(time_process, np.mean(discount_factor_nodeltadt_DisSep_Tech_processes, axis=0),label = r'$prob_{Tech}(t;x)$')
    plt.legend(loc='upper right')
    plt.ylim(0,0.10)
    # plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_DiscountFactor_Density_Sep.pdf")
    plt.close()

    plt.figure()
    plt.plot(time_process, np.mean(np.exp(discount_factor_nodelta_processes), axis=0),label = r'$Prob(t;x)$')
    plt.legend(loc='upper right')
    # plt.ylim(0,0.0012)
    # plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_DiscountFactor_Probability.pdf")
    plt.close()



    plt.figure()
    plt.plot(time_process, np.mean(discount_factor_nodeltadt_processes, axis=0),label = r'$prob_{Damage}(t;x)$')
    plt.legend(loc='upper right')
    plt.ylim(0,0.10)
    # plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_DiscountFactor_Density.pdf")
    plt.close()


    plt.figure()
    plt.plot(time_process, np.mean(W_processes, axis=0),label = r'$\log K$')
    plt.plot(time_process, np.mean(Z_processes, axis=0),label = 'Y')
    plt.plot(time_process, np.mean(V_processes, axis=0),label = r'$\log Ig$')    # plt.plot(,label = 'third term 2')
    plt.plot(time_process, np.mean(X_processes, axis=0),label = r'$\log N$')    # plt.plot(,label = 'third term 2')
    # plt.plot(,label = 'third term 3')
    plt.legend()
    plt.title(str(m0)+" "+labellist[id])
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_StateProcesss_dt.pdf")
    plt.close()


    plt.figure()
    plt.plot(time_process, np.mean(m1_processes, axis=0),label = r'$m_1$')
    plt.plot(time_process, np.mean(m2_processes, axis=0),label = r'$m_2$')
    plt.plot(time_process, np.mean(m3_processes, axis=0),label = r'$m_3$')
    plt.plot(time_process, np.mean(m4_processes, axis=0),label = r'$m_4$')
    # plt.plot(,label = 'third term 3')
    plt.legend()
    plt.title(str(m0)+" "+labellist[id])
    if m0=="Temperature":
        plt.xlim(0,50)
    plt.savefig(Plot_Dir+"/"+Filename+labellist[id]+str(m0)+"{:.3f}".format(dt)+"_MProcesss_dt.pdf")
    plt.close()
    ...
    print(f"RHS_Discounted_1={RHS_1}, RHS_Discounted_2={RHS_2}, RHS_Discounted_3={RHS_3}, RHS_Discounted_4={RHS_4}")
