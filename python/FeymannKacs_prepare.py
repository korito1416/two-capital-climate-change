...
def model_simulation_generate(xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho,psi_0,psi_1,varrho):
    ...
    Entropy_x2 = xi_d * r_1* r_2 *(Y_mat - y_bar_lower) * np.exp(r_2 / 2 * (Y_mat - y_bar_lower)**2) * (Y_mat>y_bar_lower) * np.sum( pi_d_o*(1-f_damage+f_damage*np.log(f_damage)),axis=0) 
    Entropy_x3 = xi_g * np.exp((L_mat - np.log(varrho))) * (1 - f_tech + f_tech * np.log(f_tech)) 
    ...
    print(res.keys())
    with open(Data_Dir + File_Dir+"model_tech1_pre_damage"+"_UD_prepare_PackUp_improveM_adddamage", "wb") as f:
        pickle.dump(res,f)
