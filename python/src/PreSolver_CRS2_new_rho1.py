...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    control variables (i, e, x), and calculates various coefficients used in the PDE.
    ...
    args : tuple
        Model parameters.
    controls : tuple
        Current optimal controls (i_star, e_star, x_star).
    fraction : float
    ...
    dX1, dX2, dX3, ddX1, ddX2, ddX3 : np.ndarray
        First and second order derivatives of the value function.
    ii, ee, xx : np.ndarray
        Updated control variables.
    pi_c, g_tech, h, h_k, h_j : np.ndarray
...
def hjb_pre_tech(
    ...
    """
    Solve the Hamilton-Jacobi-Bellman (HJB) equation before technology shocks.
    ...
    if smart_guess:
        ...
        # v0     = smart_guess["v0"][:,:16,:]
        # i_star = smart_guess["i_star"][:,:16,:]
        # e_star = smart_guess["e_star"][:,:16,:]
        # x_star = smart_guess["x_star"][:,:16,:]
